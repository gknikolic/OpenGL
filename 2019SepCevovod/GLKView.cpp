...
#pragma endregion
    ...
    if (nChar == VK_TAB) exit(0);
    if (nChar == VK_LEFT) m_glRenderer.rotacijaY += 10;
    if (nChar == VK_RIGHT) m_glRenderer.rotacijaY -= 10;
    if (nChar == VK_UP)	m_glRenderer.rotacijaX += 10;
    if (nChar == VK_DOWN) m_glRenderer.rotacijaX -= 10;
    if (nChar == (char)'A')	m_glRenderer.rotacijaZ -= 10;
    if (nChar == (char)'D')	m_glRenderer.rotacijaZ += 10;
    if (nChar == VK_ADD) m_glRenderer.zoom += 1;
    if (nChar == VK_SUBTRACT) m_glRenderer.zoom -= 1;
    if (nChar == VK_NUMPAD7) m_glRenderer.alphaRot -= 10;
    if (nChar == VK_NUMPAD9) m_glRenderer.alphaRot += 10;
    if (nChar == VK_NUMPAD4) m_glRenderer.betaRot -= 10;
    if (nChar == VK_NUMPAD6) m_glRenderer.betaRot += 10;
    if (nChar == VK_NUMPAD1) m_glRenderer.gamaRot -= 10;
    if (nChar == VK_NUMPAD3) m_glRenderer.gamaRot += 10;
    /*float step = 10;
    if (nChar == VK_RIGHT)
    {
